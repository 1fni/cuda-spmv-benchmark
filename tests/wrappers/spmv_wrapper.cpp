...
bool SpMVWrapper::init(MatrixData* matrix_data) {
    ...
    if (result != 0) {
        throw std::runtime_error("SpMV operator initialization failed with code: " +
...
std::vector<double> SpMVWrapper::multiply(const std::vector<double>& x) {
    ...
    // Call C run_timed function (timing output ignored)
    double kernel_time_ms;
    int result = operator_->run_timed(x.data(), y.data(), &kernel_time_ms);
    if (result != 0) {
...
MatrixDataWrapper::MatrixDataWrapper(int rows, int cols, const std::vector<Entry>& entries,
                                     int grid_size)
    : data_(std::make_unique<MatrixData>()) {
